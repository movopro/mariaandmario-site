/* Maria & Mario — final cinematic polish for desktop + mobile. */
(() => {
  const inject = () => {
    ...
    style.textContent = `
      /* Make the reveal feel like a real studio opening without using a castle/logo. */
      #mmStorybookIntro { isolation:isolate; background:#020306 !important; }
      #mmStorybookIntro::before {
        content:""; position:absolute; inset:-20%; z-index:0; pointer-events:none;
        background:radial-gradient(circle at 50% 53%,rgba(255,255,255,.95) 0 .18%,rgba(255,224,145,.32) .7%,transparent 9%),radial-gradient(circle at 50% 53%,rgba(255,210,105,.12),transparent 28%);
        opacity:0; transform:scale(.15); animation:mmCinematicFlash 7.6s cubic-bezier(.16,1,.3,1) forwards;
      }
      #mmStorybookIntro::after {
        content:""; position:absolute; inset:0; z-index:6; pointer-events:none; opacity:0;
        background:linear-gradient(115deg,transparent 0 42%,rgba(255,255,255,.9) 48%,rgba(255,216,125,.5) 50%,transparent 56%);
        transform:translateX(-120%); animation:mmCinematicSweep 7.6s ease-in-out forwards;
      }
      ...
      #mmStorybookIntro .mm-stars::before { animation:mmStarsPro2 7.6s ease-out forwards !important; }
      #mmStorybookIntro .mm-stars::after { animation:mmStarsPro3 7.6s ease-out forwards !important; }
      ...
      #mmStorybookIntro .mm-rays { animation:mmRaysPro 7.6s cubic-bezier(.16,1,.3,1) forwards !important; }
      #mmStorybookIntro .mm-orbit { animation:mmOrbitPro 7.6s cubic-bezier(.16,1,.3,1) forwards !important; }
      #mmStorybookIntro .mm-core { width:min(66vw,650px) !important; }
      #mmStorybookIntro .mm-core::before { animation:mmCorePro 7.6s cubic-bezier(.16,1,.3,1) forwards !important; }
      ...
      #mmStorybookIntro .mm-sweep { width:11px !important; filter:blur(3px) !important; animation:mmSweepPro 7.6s cubic-bezier(.16,1,.3,1) forwards !important; }
      #mmStorybookIntro .mm-caption { animation:mmCaptionPro 7.6s cubic-bezier(.16,1,.3,1) forwards !important; }
      ...
      @keyframes mmStarsPro2 { 0%,24%{opacity:0;transform:scale(.7)} 55%{opacity:.18} 100%{opacity:.34;transform:scale(1.08)} }
      @keyframes mmStarsPro3 { 0%,34%{opacity:0;transform:scale(.75)} 64%{opacity:.12} 100%{opacity:.22;transform:scale(1.04)} }
      ...
      @keyframes mmSweepPro { 0%,49%{left:-20%;opacity:0} 56%{opacity:.95} 71%{left:120%;opacity:.25} 100%{left:120%;opacity:0} }
      @keyframes mmCaptionPro { 0%,62%{opacity:0;transform:translateY(15px);filter:blur(5px)} 76%{opacity:.72;transform:none;filter:none} 100%{opacity:.72} }
      @media (max-width:760px){
        #mmStorybookIntro .mm-core{width:min(82vw,520px) !important;}
        #mmStorybookIntro .mm-initials{font-size:clamp(6.4rem,29vw,10.5rem) !important;line-height:.78 !important;}
        #mmStorybookIntro .mm-arc{width:115vw !important;height:115vw !important;}
        #mmStorybookIntro .mm-rays{width:110vw !important;}
        #mmStorybookIntro .mm-orbit{width:94vw !important;}
        #mmStorybookIntro .mm-caption{bottom:9%;font-size:.68rem;letter-spacing:.25em;}
        #mmStorybookIntro::after{background:linear-gradient(115deg,transparent 0 38%,rgba(255,255,255,.82) 48%,rgba(255,216,125,.42) 51%,transparent 60%);}
      }
  ...
  const init = () => {
    inject();
    /* Pre-warm the Web Audio context after the first user gesture, but never block navigation. */
    const warm = () => window.MMcinematicAudio?.enable?.();
